...
#define _DBC_COMMENT_MAX_LENGTH_  127           // 注释最长长度
#define _DBC_UNIT_MAX_LENGTH_     23            // 单位最长长度
...
struct DBCSignal
{
    ...
    double initialValue;                            // 初始值(原始值)
    uint8 initialValueValid;                        // 初始值是否有效
};
...
返 回 值：INVALID_DBC_HANDLE:初始化失败
          ...
          保存该值作后续使用
*/
EXTERN_C  DBCHandle FUNCCALL ZDBC_Init(uint8 disableMultiSend DEF(0), uint8 enableAsyncAnalyse DEF(0));
...
功能说明：从字符串加载DBC
参    数[in]：hDBC-句柄, DBC_Load的返回值
参    数[in]：pFileContent-文件内容字符串
参    数[in]：merge-是否合并到当前数据库; 1:不清除现有的数据, 即支持加载多个文件;0：清除原来的数据
返 回 值：true加载成功
*/
EXTERN_C  bool FUNCCALL ZDBC_LoadContent( DBCHandle hDBC, const char* pFileContent, uint8 merge );
...
注意：如果传入的是CAN多帧，每帧的data第一个字节为帧序号(从1开始)，并非数据。
      如果传入的是CANFD多帧，每帧的data变量都是数据。
参    数[in]：hDBC-句柄, DBC_Load的返回值
参    数[out]：pMsg 解析结果
参    数[in]：pObj 帧数据数组, ControlCAN传入VCI_CAN_OBJ, zlgcan根据frame_type传入can_frame或canfd_frame
参    数[in]：nCount 原始帧数据个数, 即数组大小
参    数[in]：frame_type 帧类型, 参考FT_CAN、FT_CANFD，ControlCAN不支持CANFD
返 回 值：true表示成功
*/
EXTERN_C  bool FUNCCALL ZDBC_Decode(DBCHandle hDBC, DBCMessage* pMsg, const void* pObj, uint32 nCount, uint8 frame_type DEF(FT_CAN));

/*
功能说明：根据DBCMessage编码为原始数据
注意：如果返回的是CAN多帧，每帧data的第一个字节为帧序号(从1开始)，并非数据。
      如果返回的是CANFD多帧，每帧的data变量都是数据。
参    数[in]：hDBC-句柄, DBC_Load的返回值
参    数[out]：pObj 编码的原始数据缓冲区数组, ControlCAN传入VCI_CAN_OBJ, zlgcan根据frame_type传入can_frame或canfd_frame
参    数[in,out]：pObj 缓冲区大小, 返回时为实际原始数据个数。
参    数[in]：pMsg 要编码的消息
参    数[in]：frame_type 帧类型, 参考FT_CAN、FT_CANFD，ControlCAN不支持CANFD
返 回 值：true表示成功
*/
EXTERN_C  bool FUNCCALL ZDBC_Encode(DBCHandle hDBC, void* pObj, uint32* nCount, const DBCMessage* pMsg, uint8 frame_type DEF(FT_CAN));
